...
<template>
  ...
  <div id="trail-box">
    <div class="main">
      ...
      <!-- 控制面板 -->
      <control-panel ref="control" @handlePlay="handlePlay" />
      <!-- 超速值 -->
    ...
    <!-- 底部选项卡 -->
    <foot-tab ref="foot" :pointArr="pointArr" :stopList="stopList" />
  </div>
</template>

<script>
import { getPositionByTime } from '@/api/vehicle'
...
export default {
  ...
  methods: {
    ...
    async requestInfo(startTime, endTime) {
      ...
      getPositionByTime(data).then(async res => {
        if (res.result) {
