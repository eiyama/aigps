...
// import ttxVideoAll from '@/conmon/js/swfobject-all'
...
export default {
  ...
  methods: {
    ...
    async	initPlayerExample() {
      ...
      let i = 0
      ttxVideoAll.init('cmsv6flash', w, h, params, "auto");
      this.initFlash('cmsv6flash');      
      while (i < count) {
        const id = `flash${i}`;
        aFlash[i].domId = id;
        // swfobject.embedSWF(playerSwf, id, w, h, '11.0.0', null, null, params, null)
        // const isBrowseIE = this.getBrowseVersion().indexOf("msie") != -1 ? true : false;            
        // let playerType = '';
        // if(isBrowseIE){
        //     playerType = "flash"
        // }else{
    ...
    /**
     *  判断浏览器是否为ie，是、就加载flash，否则用h5播放方式             
     */
    ...
    async initFlash(id, i, num) {
      ...
      if (        
        ...
        if (i + 1 === this.count) {
          ...
          console.log(this.isInitFinished);
        }
