<template>
	...
	>
		...
		<bm-overlay
			...
			@draw="draw($event, i)"
		>				
			<bm-info-window  :position="v.pt" :height="255" :width="330"  :show="v.show"  :closeOnClick="false" >				
				<div class="vehinfoW">	
					<veh-info-win  :info="v" @handleColse="handleColse" @handleBtn="handleBtn" @handleActive="handleActive" />
				</div>
			</bm-info-window>					
			<!-- <veh-info-win v-show="v.show"   :info="v" @handleColse="handleColse" @handleBtn="handleBtn" @handleActive="handleActive" /> -->
		</bm-overlay>
		<!-- marker点聚合 -->
...
<style scoped >
	...
	.bm-view >>> .BMap_bubble_content .arrow {
		display: none !important;
	}
	/* 内容 */
...
export default {
	...
	created() {
		this.lines = [];
		eventBus.$on('handleMonitorRow', this.handleMonitorRow);  		 
	},
	mounted(){	 
		// eventBus.$emit('setmap', this.$refs.map);
	},
	beforeDestroy() {
		eventBus.$off('handleMonitorRow', this.handleMonitorRow); 
		// const data = this.getData()
	...
	methods: {
		 ...
		 */
		 handleMonitorRow(handleArr) {					 
				...
				if (obj && obj.location) {
					...
					delete info.location
					this.setInfoWin(info)
				}
				// 是子节点，将选中的车辆居中
				this.setMarkerAll(arr, false)			
		},
		/**
		 * 设置marke点
		...
		setMarkerAll(arr, boole) {
			...
			for (let [i, v] of pointArr.entries()) {
				...
				// const BDPt = new BMap.Point(gpsWebLng,gpsWebLat);			
				v.BDPt = BDPt
				pointArr[i] = v
				const mData = {
					...BDPt,
    ...
    getInfo(data) {
      ...
      return obj
		...
		setInfoWin(info) {
			...
			return new Promise(async resolve => {
				...
				for (const v of keys.values()) {
					if (info[v.key] || !v.required || v.key == 'driver') {
						const obj = {}
	          			obj.label = v.name
		...
		// 坐标转换
		switchPt(oldpoint,type){
			let newPoint = oldpoint;
			for (const [i, v] of newPoint.entries()) {			 					
				if (v.pt instanceof Array) {
					let bdpt = [];				  
					 for(const pt of v.pt){
						bdpt.push(getLngLat(pt.lng, pt.lat, 'bd09togcj02'));
					 }	
					 v.pt = bdpt;				
					// 坐标转换					 
				} else {		
					v.pt = 	getLngLat(v.pt.lng, v.pt.lat, 'bd09togcj02')						 					
				}				
			}	
		...
		setSpot(arr2, type, isArr) {
			...
			for (const v of arr2.values()) {
				...
				for (const e of markLatLngs.values()) {
					const { lng, lat } = e
					const lngLat = getLngLat(lng, lat, 'gcj02tobd09')
					// const lngLat = new BMap.Point(lng,lat)
					data.pt.push(lngLat)
				}
