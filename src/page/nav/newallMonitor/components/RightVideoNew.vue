<template>
  <div v-if="isShow" :class="isrvideoShow ? 'videos-transition-start' : 'videos-transition-end'" class="right-videos videos">
    <div class="active-bar">
      ...
      <span style="float: right;" @click="handleVideosHide" class="el-icon-close"></span>
    </div>
    <div class="video-box" ref="flash">
      <div class="flash-box" id="cmsv6flash"></div>
...
.video-box .flash-box  >>>  .video-basis-2{
  width: 100% !important;
}
...
export default {
  ...
  methods: {
    ...
    async	initPlayerExample() {
      ...
      let i = 0
      ttxVideoAll.init('cmsv6flash', w, h, params);        
      while (i < count) {
        const id = `flash${i}`;
        aFlash[i].domId = id;
        this.initFlash('cmsv6flash',i);                        
        i++
      }
    },
    ...
    playVideo(id,i) {            
      ...
      // 播放视频      
      swfobject.getObjectById(id).startVideo(i, jsession, vehTerminalNo, channelId, 1 , true)     
    },
    /**
     * 点击中间视频关闭
     */
     ...
     */
    handleVideosHide() {
      // this.handleShowCenVideo()
      this.stopAll(true)       
      this.isrvideoShow = false
      this.clearData(false, true)
      // 关闭之前暂停所有的视频
