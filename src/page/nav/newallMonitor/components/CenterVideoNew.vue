<template>
  <div v-show="isShow" class="main-vedio">
    ...
    <div class="center-videos videos">
      <div class="active-bar">
        ...
        <span @click="handleVideosCenHide" class="el-icon-close hide"></span>
      </div>
      <!-- 视频 -->
  ...
  methods: {
    ...
    async initFlash(id, num) {
      ...
      if (
        ...
        console.log(JSON.stringify(swfobject.getObjectById(id)))
        await this.$nextTick()
    ...
    handlePlay(i) {
      ...
      swfobject.getObjectById('flash').stopVideo(i)
      // 设置窗口标题
      const title  = `${vehPlate}-${name}`
      swfobject.getObjectById('flash').setBufferTime(i, 2)
      swfobject.getObjectById('flash').setBufferTimeMax(i, 6)
      swfobject.getObjectById('flash').setVideoInfo(i, title)
      // 播放视频
      swfobject.getObjectById('flash').startVideo(i, jsession, vehTerminalNo, channelId, bitstream, true)
    },
    /**
     * 遍历一组视频盒子出来
   ...
   /**
    ...
    async handleChangePageSize(num) {
      ...
      for (const [i, v] of aFlash.entries()) {
        v.isReplace = false
        if (v.switch && !v.url) {
          this.reSetVideo('flash', i)
          v.switch = false
        }
      }
      this.count = num

      this.setWindowNum('flash')
      await this.$nextTick()
      for (const [i, v] of aFlash.entries()) {
        if (v.switch && i >= count) {
          this.handlePlay(i) 
