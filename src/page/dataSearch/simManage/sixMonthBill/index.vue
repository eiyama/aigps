...
import { getCardBeforeSixMonthInfo } from '@/api/dataSearch/sim'
...
export default {
  ...
  data() {
    const searchList = getCopyObj(this.$t('sixMonthBill.searchList'));
    const tableTitle = this.$t('sixMonthBill.tableTitle');
    return {
      searchList,
      tableTitle,
      tableData: [],
  ...
  methods: {
    ...
    requestInfo() {
      ...
      getCardBeforeSixMonthInfo(data).then(res => {
        if (res.result) {
