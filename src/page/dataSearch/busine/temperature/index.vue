...
export default {
  ...
  name: 'Temperature',
  data() {
    const searchList = getCopyObj(this.$t('temperature.searchList'))
    const tableTitle = this.$t('temperature.tableTitle');
    const pageing = {
      curPage: 1,
      limit: 600,
      totalCount: 0,
  ...
  methods: {
      ...
      getReportPosition(data).then(res => {
        if (res.result) {
          ...
          for(const v of list.values()) {
            ...
            v.temperature = getDecimalN(extensionData.ConvertTemperature);
          }
          this.tableData = list;
     ...
     */
    getOpt(list) {
      const arr = getCopyObj(this.$t('temperature.options'));
      ...
      opt.timeArr = [];
      for (const v of list.values()) {
        if (v.temperature < 6535.5) {
          opt.series[0].data.push(v.temperature);
    ...
    exportFile() {
      ...
      getTemperatureExcel(data).then(res => {
        if (res.result) {
