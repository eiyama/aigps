      ...
      <template>
        <el-tabs @tab-click="tabClick" style="display: flex;flex-direction: column;flex: 1;overflow: hidden;" v-model="activeName">
          ...
          <el-tab-pane :label="$t('otherText.table')" name="table">
            <my-table ref="table" :isPageing="false" :tableData="tableData" :tableTitle="tableTitle" @handleCell="getAddress" />
          </el-tab-pane>
        </el-tabs>
      </template>
      <div class="block">
...
import { getReportPosition, getTemperatureExcel } from '@/api/dataSearch/busine'
...
export default {
  ...
  name: 'load',
  data() {
    const searchList = getCopyObj(this.$t('load.searchList'))
    const tableTitle = this.$t('load.tableTitle');
    const pageing = {
      curPage: 1,
      limit: 600,
      totalCount: 0,
  ...
  async created() {
    ...
    let code, url;
    if (this.curMarker) {
      code = this.curMarker.code;
      url = this.curMarker.url;
      this.componentInit();
    } else {
  ...
  methods: {
      ...
      getReportPosition(data).then(res => {
        if (res.result) {
          ...
          for (const v of list.values()) {
            ...
            v.carState = v.gpsSpeed ? this.$t('otherText.run') : this.$t('otherText.stop');
            v.address = this.$t('button.queryAddress');
            const extensionData = v.extensionDataObj || {};
            v.load = extensionData.Load ? getDecimalN(extensionData.Load  * v.loadFactor) : 0;

            opt.series[0].data.push(v.gpsSpeed);
            opt.series[1].data.push(v.load);
     ...
     */
    getOpt(list) {
      const arr = getCopyObj(this.$t('load.options'));
    ...
    getAddress(row) {
      ...
      this.info.lng = row.gpsWebLng;
      this.info.lat = row.gpsWebLat;
