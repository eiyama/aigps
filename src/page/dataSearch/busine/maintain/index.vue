...
import { getMaintenanceData } from '@/api/dataSearch/busine'
...
export default {
  ...
  data () {
    const searchList = getCopyObj(this.$t('maintain.searchList'))
    const tableTitle = this.$t('maintain.tableTitle')
    return {
      searchList,
      tableTitle,
      tableData: [],
  ...
  methods: {
    ...
    async requestInfo () {
      ...
      try {
        const res = await getMaintenanceData(data)
        const { result } = res
