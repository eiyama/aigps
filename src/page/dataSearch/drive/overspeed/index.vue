...
export default {
  ...
  data() {
    const searchList = getCopyObj(this.$t('overspeed.searchList'));
    const tableTitle = this.$t('overspeed.tableTitle');
    return {
      searchList,
      tableTitle,
      tableData: [],
  ...
  methods: {
    ...
    query() {
      ...
      // 超速阈值必须是整数但是可以为空
      const limitSpeed = this.searchList.limitSpeed.model;
      if (!selectTime || !selectTime.length) {
        Message.error(this.$t('error.selectTimes'));
        return;
      } else if (limitSpeed && !isNumber(limitSpeed)) {
        Message.error(this.$t('error.limitSpeed'));
        return;
      }
      this.requestInfo();
    },
    ...
    exportFile() {
      ...
      if (length) {
        exportFile(length, this.$t('overspeed.curTitle'));
      } else {
