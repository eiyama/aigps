...
import { getCopyObj, getDateHMS, getParse, getDateStr } from '@/utils/util'
...
export default {
  ...
  data() {
    const searchList = getCopyObj(this.$t('fatigueDriving.searchList'));
    const tableTitle = this.$t('fatigueDriving.tableTitle');
    return {
      searchList,
      tableTitle,
      tableData: [],
  ...
  methods: {
    ...
    exportFile() {
      ...
      if (length) {
        exportFile(length, this.$t('fatigueDriving.curTitle'));
      } else {
