...
import { getCopyObj, getDateStr, getParse } from '@/utils/util'
...
export default {
  ...
  data() {
    const searchList = getCopyObj(this.$t('swerve.searchList'));
    const tableTitle = this.$t('swerve.tableTitle');
    return {
      searchList,
      tableTitle,
      tableData: [],
  ...
  methods: {
      ...
      getAlarmRecode(data).then(res => {
        if (res.result) {
          ...
          for (const v of list.values()) {
            ...
            v.position = v.direction == 1 ? this.$t('otherText.right') : this.$t('otherText.left');
    ...
    exportFile() {
      ...
      if (length) {
        exportFile(length, this.$t('swerve.curTitle'));
      } else {
