...
<template>
  <div class="content">
    ...
    <my-table ref="table" :isCheck="true" :pageSize="[50, 100, 500, 1000,2000]" :tableData="tableData" :tableTitle="tableTitle" @request="requestInfo" @handleCell="handleCell" @select="changeCheck" @selectAll="selectAll" @handleRow="changeCheck" />
  </div>
</template>
...
import { getAlarmDetailNew, getAlarmDetailNewExport , getDevicerAlarmNew ,exportDeviceAlarm } from '@/api/dataSearch/deviceAlarm'
...
export default {
  ...
  methods: {
     ...
     */
    requestInfo(argumentData) {
      if (!argumentData) argumentData = this.getData();
      let ids =  argumentData.ids || [];
      if(ids){
        ids = ids.split(',');
      }
      let alarmCategoryArr = [] ,alarmDisplayArr = [];
      if(argumentData && argumentData.alarmCategory && argumentData.alarmCategory.length){
          alarmCategoryArr = (argumentData.alarmCategory).split(",");
      }
      if(argumentData && argumentData.alarmType && argumentData.alarmType.length){
          alarmDisplayArr = (argumentData.alarmType).split(",");
      } 
      const handle = argumentData.handle === '' ? null : argumentData.handle == '1' ? true : false;         
      const duration = argumentData.duration * 1;     
      let data = {
        "vehTerminalNos": ids,
        "startTime": argumentData.begin,
        "endTime": argumentData.end,
        "alarmCategory": alarmCategoryArr,
        "alarmDisplay": alarmDisplayArr,
        "alarmHandleStatus":  handle,
        "alarmDuration": duration,
        "alarmLevel": 0,
        "pageOption": {
          "pageIndex": argumentData.curPage,
          "pageSize": argumentData.limit,
          "total": argumentData.totalCount
        }
      }
      //  console.log(data)
      const me = this;   
      getDevicerAlarmNew(data).then(res => {
        if (res.result) {
          const list = res.result.getDeviceAlarmNewDtos || []
          me.oldData = argumentData
          for (const v of list.values()) {
            ...
            if(v.alarmHandleInfo){
                ...
                v.alaStateText = v.alarmHandleInfo.alarmHandleStatus ? this.$t('otherText.handleYes') : `${this.$t('otherText.handleNo')}，${this.$t('button.clickHandle')}`;                
            }else{
              v.alarmStatus = false; 
    ...
    handleCell(row, key) {
      ...
      // 点击结束
        ...
        eventBus.$emit('handleClickNew', row);
      }
    },
    /**
     * 点击处理按钮批量处理
    ...
    handleAlarms() {      
      ...
      data.ids = ids.join()
      eventBus.$emit('handleClickNew', data)
    },
