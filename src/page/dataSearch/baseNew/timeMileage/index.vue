...
import { getReportMileagePeriod ,GetMileage,exportMileage} from '@/api/dataSearch/base'
...
export default {
  ...
  methods: {
    ...
    getAddress(row) {
      ...
      if (row.mileage <= 0) {
        ...
        const startObj = {
          ...
          lng: row.startGpsWebLng,
          lat: row.startGpsWebLat,
          title: row.vehPlateNo,
    ...
    async exportFile() {
     ...
     try {
        const res = await exportMileage(data)
        if(res.result){
