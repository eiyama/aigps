...
import { exportPositionReport, getPositionWithPage } from '@/api/dataSearch/base'
...
export default {
  ...
  methods: {
    ...
    requestInfo() {
      ...
      getPositionWithPage(data).then(res => {
        if (res.result) {
          const list = res.result.getPositionWithPageDtos;
          for (const e of list.values()) {
            e.gpstime = getDateStr(e.gpstime);
            e.creationTime = getDateStr(e.createTime);
            // 扩展字段展示定位类型
            ...
            e.gpsWebLng = e.gcj02Coordinate.lng;
            e.gpsWebLat = e.gcj02Coordinate.lat;
          }
          this.tableData = list;
    ...
    exportFile() {
      ...
      exportPositionReport(data).then(res => {
        if (res.result) {
