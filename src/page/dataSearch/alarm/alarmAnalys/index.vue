...
import { getAlarmReportNew, getAlarmReportTrendNew, getAlarmReportExcel } from '@/api/dataSearch/alarm'
...
export default {
  ...
  methods: {
    ...
    requestInfo() {
      ...
      getAlarmReportNew(data).then(res => {
        if (res.result) {
          // 把当前行的数据清空
    ...
    getDayQueryInfo(data) {
      getAlarmReportTrendNew(data).then(res => {
        if (res.result) {
    ...
    exportFile() {
      ...
      const data = this.getrequestData();
      getAlarmReportExcel(data).then(res => {
        if (res.result) {
          const result = res.result;
          location.href = `${process.env.VUE_APP_BASEURL}${result.fileName}`
        }
      })
      .catch(err => {
      ...
      if (Array.isArray(arr) && arr.length > 0) {
        for (const e of arr.values()) {
          ...
          if (obj) {
            ...
            } else if (!level && v.other2 > 0) {
              v.other2 = v.other2 - e.alarmCount;
            }
          }
        }
      }
