    ...
    <el-table :id="id" stripe ref="table"  border :data="tableData" :height="height" size="mini" :show-summary="showSummary" :summary-method="summary"
      ...
      <el-table-column align="center" v-for="(v,index) in tableTitle" :prop="v.prop" :min-width="v.width" :label="v.label" :key="index" :sortable="!!v.sort" > 
        <template slot-scope="scope">
          ...
          <span v-else-if="!!v.disabled "  class="select-color">                       
            <el-tooltip v-if="scope.row[v.prop].isActive" effect="dark" content="已激活，点击禁用" placement="top-start">
              <span @click.stop="handleCell(scope.row, 'isActive')" >
                  禁用 
              </span>
            </el-tooltip> 
            <el-tooltip  v-else effect="dark" content="已禁用，点击激活" placement="top-start">
                <span @click.stop="handleCell(scope.row, 'isActive')" >
                   激活 
                </span>
            </el-tooltip>             
            <el-tooltip  v-if="scope.row[v.prop].isPublic"  effect="dark" content="已公用，点击私有" placement="top-start">
                <span @click.stop="handleCell(scope.row, 'isPublic')" style="margin-left:5px" >
                   私有 
                </span>
            </el-tooltip>
             <el-tooltip  v-else  effect="dark" content="已私有，点击公用" placement="top-start">
                <span @click.stop="handleCell(scope.row, 'isPublic')" style="margin-left:5px" >
                   公用 
                </span>
            </el-tooltip>
...
<script>
import { Table, TableColumn, Form, FormItem, Pagination ,Tooltip } from 'element-ui'
