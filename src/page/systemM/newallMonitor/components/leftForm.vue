        ...
        <el-form  :disabled="disabled"
          ...
          <el-form-item v-show="!v.isHide"
            ...
            <el-date-picker :disabled="!!v.disabled"
                            ...
                            type="date"
                            :picker-options="pickerOptions"
                            :placeholder="v.placeholder ||  $t('placeholder.times')"></el-date-picker>
            <!-- 固定时间 -->
          ...
          <div v-if="showAddress">
            <el-form-item  class="input-bar" label="选择区域:" :label-width="'120px'" >
              <el-select v-model="addList.provinceName" placeholder="请选择省" class="input-max" @change="getcity(addList.provinceName,true)"  >
                <el-option v-for="item in province" :key="item.code" :label="item.name" :value="item.code">
                </el-option>
              </el-select>
                                         
              <el-select style="margin-top: 10px;" v-model="addList.cityName" placeholder="请选择市" class="input-max" @change="getarea(addList.cityName,true)"  >
                <el-option v-for="item in citys" :key="item.code" :label="item.name" :value="item.code">
                </el-option>
              </el-select>            
                       
              <el-select style="margin-top: 10px;" v-model="addList.areaName" placeholder="请选择区" class="input-max" @change="postData(addList.areaName)" >
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rules: {
          ...
          fenceRule: [
            { required: true, message: '请选择围栏规则', trigger: 'blur' },           
          ],
          fencemarkType:[
  ...
  methods: {
    getcity(val){ 
      this.addList.cityName = ''; 
      this.addList.areaName = ''; 
      address.map(item=>{
        if(val == item.code){
          this.citys = item.childs;
          this.name = item.name;
        }
      })
      this.code = val;
    ...
    busvalue(){
      console.log(this.name)
       eventBus.$emit("addressObj",{name:this.name,code:this.code,markType:this.markType});
    },
		/**
		 * 点击关闭树
		 */
