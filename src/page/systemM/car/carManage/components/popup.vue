  ...
  <div class="popup">
    ...
    <el-dialog :title="$t('carManage.uploadInfo.title')" :close-on-click-modal="false" @close="uploadPopupClose"
      ...
      <div class="upload-box">
        ...
        <el-select class="input margin-bottom" size="small" v-model="uploadInfo.deviceTypeId"
          :placeholder="$t('carManage.uploadInfo.msgDeviceType')">
          <el-option v-for="(item,index) in selectList" :key="index" :label="item.devtName"
            :value="item.id"></el-option>
        </el-select>
        <!-- 日期 -->
        <el-date-picker size="small" value-format="yyyy-MM-dd" v-model="uploadInfo.expired" class="input"
         type="date" :clearable="false" :disabled="!isRoot" :placeholder="$t('placeholder.times')"></el-date-picker>
      </div>
...
import { Upload, Input, Button, Dialog, Tree, DatePicker, Select, Option, Scrollbar, Message } from 'element-ui'
import { getNYearLater, parentId } from '@/utils/data'
// import { copyObj } from '@/utils/util'
import { uploadVehicle } from '@/api/sysetsmM/car'
...
export default {
  ...
  created() {
    const { userInfo } = this
    this.isRoot = userInfo.id === parentId
  },
  mounted() {
  ...
  methods: {
     ...
     */
    downloadTemplate() {
      location.href = process.env.VUE_APP_BASEURL + 'report/template.xlsx';
    },
    /**
     * 点击导入
     */
    ...
    uploadPopupClose() {
      ...
      this.$set(this.uploadInfo, 'expired', getNYearLater(1));
    },
    /**
     * 选择部门树的输入框触发焦点
     */
    ...
    uploadConfirm() {
      ...
      const deviceTypeId = this.uploadInfo.deviceTypeId;
      const expired = this.uploadInfo.expired;
      if (!dptId) {
        Message.error(i18n.msgMenu);
        return;
      } else if (!deviceTypeId) {
        Message.error(i18n.msgDeviceType);
        return;
      } else if (!this.formData) {
        Message.error(i18n.msgFile);
        return;
      }
      uploadVehicle(this.formData, { dptId, deviceTypeId, expired }).then(res => {
        if (res.success) {
          Message({
            type: 'success',
