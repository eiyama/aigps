  ...
  methods: {
    ...
    requestInfo(oldSearchVal) {
      ...
      const data =  {
        ...
        "alarmHandleStatus": oldSearchVal.state || handle,
        "alarmDuration":  oldSearchVal.duration || duration || 0,
        "pageOption": {
      ...
      getDevicerAlarmNew(data).then(res => {
        if(res.result) {
          ...
          for (const v of arr.values()) {
            ...
            v.alaStateText = v.alarmHandleStatus ? this.$t('otherText.handleYes') : `${this.$t('otherText.handleNo')}，${this.$t('button.clickHandle')}`;
          }         
          this.tableData = arr;
